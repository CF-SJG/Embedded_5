...
#### - 초음파 센서
<p>
   초음파 신호를 송신(TRIG 핀)한 후 반사되어 돌아오는 시간을 측정(ECHO 핀)<br>
   반환 시간(travel time)을 기반으로 거리를 계산: 거리(cm) = travel time(μs)/58<br>
   거리 범위가 16~22cm사이일 때 장애물이 존재한다고 판단하면, choState 를 업데이트한다.<br>
</p>
#### - 빗물 감지 센서
